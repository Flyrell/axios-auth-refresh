...
export default function createAuthRefreshInterceptor(
    ...
    return instance.interceptors.response.use(
        ...
        (error: AxiosError) => {
            ...
            // Loop protection: track how many times this request has been retried
            const config = error.config as any;
            if (config) {
                config.__authRefreshRetryCount = (config.__authRefreshRetryCount || 0) + 1;
                if (config.__authRefreshRetryCount > mergedOptions.maxRetries) {
                    return Promise.reject(error);
                }
            }
            ...
            // If a refresh call does not exist, create one
            const refreshing = createRefreshCall(error, refreshAuthCall, cache);

            // Create an interceptor that will bind all the others requests until refreshAuthCall is resolved
